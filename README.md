...
### Session 6 — a training-data execution system that proves what it consumed

Session 5 wrote the recipe; Session 6 builds the **machine that executes it** — and proves the run is correct,
reproducible, and auditable. A small, fully-deterministic pipeline runs the whole path *documents → tokenized
shards → manifests → mixture schedule → packing → batches → (fake) training → consumption + learning ledgers →
checkpoint → crash → resume → replay → fork → audit* behind one command (`python run_demo.py`), and writes a
`submission_artifacts/` bundle whose evidence is **generated by the code, not hardcoded**.

The whole system rests on one invariant: **every batch is a pure, reconstructable function of `(master_seed,
compiled schedule, shard set, sampler state)`** — so resume/replay/fork are *provable*, by recomputing the batch
and comparing its content hash. The batch payload is integer-only, so hashes are machine-portable; the only
float math is a throwaway bigram model whose initial loss is **ln(V)** — the lecture's −ln(1/vocab).

| | Result (from the actual run) |
|---|---|
| **Crash → resume** | crash at step 25 → resume from checkpoint 20 → the **entire resumed stream is hash-identical** to a clean run (no skipped/repeated batch) |
| **Replay** | reconstruct an interval from the ledger → batch hashes match the originals |
| **Fork** | branch from a checkpoint with a new seed → the stream **diverges** yet stays self-consistent |
| **Firewall / floors / OPUS** | eval shards withheld and no eval doc reaches a loss-bearing batch; protected floors always met; OPUS accept/reject/defer with floor overrides on Indic/agentic/reasoning |

**→ Full system + one-command demo: [`session-06-data-execution/`](./session-06-data-execution/)** — the
Session 6 submission (GitHub repo + `run.log` / `evidence.json` / `evidence.md`). 11 invariant tests, numpy-only.
